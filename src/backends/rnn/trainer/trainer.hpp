...
class NetworkTrainer {
    public:
        ...
        void train(const BehaviourList& x, const BehaviourList& y) {
            ...
            std::ranges::shuffle(randomized_examples, std::mt19937(std::random_device()()));
            ...
            for (int epoch = 0; epoch < num_epoch; epoch++) {
                ...
                std::ranges::shuffle(examples_for_ep, std::mt19937(std::random_device()()));

                int at_ep_ex = 0;
                int batch = 0;

                #pragma omp parallel while
                while (at_ep_ex < examples_for_ep.size()) {
                    ...
                    for (int ex = 0; ex < num_ex_in_batch; ex++) {
                        ...
                        std::unique_ptr<arma::mat> tmpU; // temp U
                        std::unique_ptr<arma::mat> tmpV; // temp V
